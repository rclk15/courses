...
Ricky Cheah
7/4/2020
Lab 6 - File is modified from Lab 4 to work with HuffmanTrees. 
Heap Class
"""
class Heap(object):
    '''
    ...
    def percolateUp(self, comparable: int):
        ...
        while parentIndex > -1 and currentIndex != parentIndex :
            parentValue = self.getElement(parentIndex).getFrequency()
            if parentValue > comparable.getFrequency():
...
#            print('whie')
            ...
            if rightChildIndex < self.size(): # making sure right child is available
                ...
                if leftChild.getFrequency() >= rightChild.getFrequency():
            ...
            if comparable.getFrequency() >= self.getElement(smallestChildIndex).getFrequency():
