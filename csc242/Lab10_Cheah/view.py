...
class GraphDemoView(object):
    ...
    def _getFromKeyboard(self):
        ...
        endLabel = input("Enter the end label: ")
        print(self._model.createGraph(rep, startLabel, endLabel))

    def _getFromFile(self):
        """Inputs a description of the graph from a file
        and creates the graph."""
        fileName = input("Enter the file name with extension (.gph): ")
        rep = ""
        print(fileName)
