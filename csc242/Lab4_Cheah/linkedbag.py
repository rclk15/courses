...
class LinkedBag(object):
    ...
    def clone(self):
        """
        When called on an existing LinkedBag object, returns an exact LinkedBag 
        copy (elements will be in the same order)
        """
        
        newBag = LinkedBag()
        #because our add method adds new items at the front of the new LinkedList
        #we need to reverse the order of the items being added.
        tempList = [] 
        
        for item in self: #this reverses the order of items to be added
            tempList = [item] + tempList
            
        for item in tempList: #adds item to new LinkedList
            newBag.add(item)
        return newBag
